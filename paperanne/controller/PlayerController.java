...
public class PlayerController {
    ...
    public void update(PlayerModel m, ImageView v, Set<KeyCode> keys, double groundY, List<ImageView> papers,
                       ...
                       PaperModel targetPaper,List<Achievement> achievementList) {
        ...
        else if(m.sizeState == 1){
            mult = 1.1;
        }
        else if(m.sizeState == -1){
            mult = 0.7;
        }
        m.velY += m.gravity * mult;
        m.y += m.velY;
        ...
        if (lever != null && v.getBoundsInParent().intersects(lever.getBoundsInParent())) {
            ...
            if (currentEKey && !isEPressed) {
                if (lever.getImage() == leverLeft) {
                    ...
                    // 状态A -> 状态B：纸片长高
                    if (targetPaper != null) {
                        targetPaper.setHeight(150);
                        targetPaper.setY(groundY - 150); // 向上生长，确保底部贴地
                    }
                } else {
                    lever.setImage(leverLeft);
                    // 状态B -> 状态A：纸片缩回
                    if (targetPaper != null) {
                        targetPaper.setHeight(50);
                        targetPaper.setY(groundY - 50);
                    }
                }
            }
        }
        ...
        for (Achievement achievement : achievementList){
            ImageView av = achievement.paint();
            if(av.isVisible() && v.getBoundsInParent().intersects(av.getBoundsInParent())){
                if(!achievement.isPickedUp()){
                    achievement.setPickedUp();
