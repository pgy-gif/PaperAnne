...
public class Enemy {
    ...
    public Enemy(double startX, double startY, double minX, double maxX, PaperModel platform) {
        ...
        if (platform != null) {
            this.lastPlatformX = platform.getX();
        }
    ...
    public void update(PlayerModel player) {
        // --- 1. 如果绑定了纸片，实时更新边界和高度 ---
        if (platform != null) {
            double currentPlatformX = platform.getX();
            double deltaX = currentPlatformX - lastPlatformX;

            // 关键：让敌人的 X 坐标跟随纸片同步移动
            this.x += deltaX;
...
//
//        // --- 3. 吓退逻辑 (变大且在同层且在视线内) ---
//        if (player.sizeState == 1 && isSamePlane && dist < sightRange) {
//            isScared = true;
//            // 尝试向反方向移动
//            if (player.x > this.x) {
//                moveLeft(1.5); // 向左跑
...
//        }
//        // --- 4. 追击逻辑 (同层 && 视线内 && 没死 && 没被吓跑) ---
//        else if (isSamePlane && dist < sightRange && !player.isDead) {
//            isScared = false;
//            if (player.x > this.x) {
//                moveRight(1.0); // 向右追
//            } else {
